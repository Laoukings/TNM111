...
document.addEventListener('DOMContentLoaded', function () {
    ...
    Promise.all([
        ...
        document.getElementById("episode-select").addEventListener("change", function () {
            ...
            simulation.force("link", d3.forceLink(currentData.links).id(d => d.index));
            simulation.alpha(1).restart();

            // Update the links, nodes, and labels
            links.data(currentData.links).exit().remove();
            links.enter()
                .append("line")
                .attr("stroke", "#999")
                .attr("stroke-width", d => linkWidthScale(d.value))
            ...
            nodes.data(currentData.nodes).exit().remove();
            nodes.enter()
                .append("circle")
                .attr("r", d => radiusScale(d.value))
                .attr("fill", d => d.colour)
            ...
            labels.data(currentData.nodes).exit().remove();
            labels.enter()
                .append("text")
                .text(d => d.name)
                .attr("font-size", "10px")
        ...
        document.getElementById("node-radius-slider").addEventListener("input", function () {
            ...
            nodes.attr("r", d => radiusScale(d.value));
        });
        ...
        document.getElementById("link-width-slider").addEventListener("input", function () {
            ...
            links.attr("stroke-width", d => linkWidthScale(d.value));
        });
